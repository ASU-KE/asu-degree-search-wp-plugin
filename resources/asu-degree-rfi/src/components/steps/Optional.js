...
function getStateOptions( resultsArrayOfObjects, formikValues ) {
	...
	if (
		! ( formikValues.Country === 'US' || formikValues.Country === 'CA' )
	) {
		return [
			{
				key: '0',
	...
	const arrayOfStates = states.filter(
		( country ) => country.stateCode !== 'IGD'
	);
...
async function fetchCountries(
	...
	return fetch( serviceUrl )
		...
		.then( ( result ) => {
			...
			const results = optionsCallback(
				resultsArrayOfObjects,
				formikValues
			);
			return results;
		} )
