...
async function fetchDegreesData(
	...
	if ( Campus === 'ONLNE' ) {
		program =
			CareerAndStudentType === 'Readmission'
				? 'graduate'
				: 'undergraduate';
...
const ProgramInterest = ( props ) => {
	...
	useEffect( () => {
		...
		if ( values.Campus === 'ONLNE' ) {
			...
			// ASUOnline mapping
			const poiOptions = degreeDataProcessed.map(
				( program, index ) => ( {
					key: index.toString(),
					value: program.code,
					text: program.title,
	...
	return (
		<>
			<p className="rfi-step1-intro">
				To learn more about ASU or a specific program, fill out the form
				below then check your email.
			</p>
			{
			...
			<RfiSelect
				...
				requiredIcon={
					! props.programOfInterestOptional ||
					values.Campus === 'ONLNE'
				}
				required={
					! props.programOfInterestOptional ||
